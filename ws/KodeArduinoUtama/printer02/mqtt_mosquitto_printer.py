...
"""
========================================================================
MQTT Barcode Printer Service - Mosquitto Lokal Edition
========================================================================
Untuk menggunakan Mosquitto broker lokal instead of cloud broker
========================================================================
"""
...
# ========================================================================
# LOAD CONFIGURATION - Support both mqtt_config.ini dan mosquitto_config.ini
# ========================================================================

# Try mosquitto_config.ini first, fallback to mqtt_config.ini
CONFIG_FILE = os.path.join(os.path.dirname(__file__), "mosquitto_config.ini")
...
try:
    ...
    BROKER_ADDRESS = cfg.get("MQTT", "BROKER_ADDRESS", fallback="localhost")
    BROKER_PORT = cfg.getint("MQTT", "BROKER_PORT", fallback=1883)
    CLIENT_ID = cfg.get("MQTT", "CLIENT_ID", fallback="barcode_printer_mosquitto")
    ...
    print(f"[CONFIG] Configuration loaded from: {os.path.basename(CONFIG_FILE)}")
...
def init_printer():
    ...
    try:
        ...
        # Set print quality
        try:
            printer_dll.SetDensity(15)
            logger.debug("SetDensity(15)")
        except:
            pass
        
        try:
            printer_dll.SetSpeed(3)
            logger.debug("SetSpeed(3)")
        except:
            pass
...
def cetakBarcode01(id1, nama, ws):
    """Fungsi cetak barcode (sama seperti mqtt_cloud_printer.py)"""
    try:
        if printer_dll is None:
        ...
        # STRATEGY 0: PrintBarcode (Code128)
        if not print_success:
            try:
                printer_dll.PrintChargeRow()
                printer_dll.PrintBarcode(b_string1, 73, 2, 2, 0, 0)
                print_success = True
                logger.info("Barcode printed (PrintBarcode - Code128)")
            except Exception as e:
                logger.debug(f"PrintBarcode failed: {str(e)[:50]}")
        
        # STRATEGY 1: Type 2 (EAN/UPC)
        try:
            printer_dll.PrintChargeRow()
            qrcode_result = printer_dll.Print1Dbar(2, 60, 1, 2, 4, b_string1)
            print_success = True
            logger.info("Barcode printed (Type 2)")
        except Exception as e:
            logger.debug(f"Type 2 failed: {str(e)[:50]}")
        
        # STRATEGY 2: Type 73 (Code128)
        if not print_success:
            try:
                printer_dll.PrintChargeRow()
                qrcode_result = printer_dll.Print1Dbar(73, 60, 2, 2, 0, b_string1)
                print_success = True
                logger.info("Barcode printed (Type 73)")
            except Exception as e:
                logger.debug(f"Type 73 failed: {str(e)[:50]}")
        
        # STRATEGY 3: QR Code
        if not print_success:
            try:
                printer_dll.PrintChargeRow()
                qrcode_result = printer_dll.Print2Dbar(6, 5, b_string1)
                print_success = True
                logger.info("QR Code printed (Type 6)")
            except Exception as e:
                logger.debug(f"QR Code failed: {str(e)[:50]}")
        
        if not print_success:
            logger.warning("Barcode print failed")
...
def main():
    ...
    print("")
    print("="*70)
    print("MQTT BARCODE PRINTER SERVICE - MOSQUITTO LOKAL")
    print("="*70)
    print("")
    ...
    try:
        ...
        # Set auto-reconnect dengan exponential backoff
