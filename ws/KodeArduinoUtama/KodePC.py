...
def lcd_msg_0():
    file = open(r"C:\ws\KodeArduinoUtama\lcd_msg.txt","w")
    file.write("0")
    file.close()

def msg_0(): #Buzzer
    file = open(r"C:\ws\KodeArduinoUtama\buzzer_msg.txt", 'w')
    file.write("0")
    file.close()
...
ArduinoUnoSerial = serial.Serial('COM3',9600)       #membuat koneksi serial ke port 5 dengan  nama ArduinoUnoSerialData                                                             #wait for 2 secounds for the communication to get established
...
while (1==1):
    ...
    fileBuzz = open(r"C:\ws\KodeArduinoUtama\buzzer_msg.txt","r")
    ...
    fileLCD = open(r"C:\ws\KodeArduinoUtama\lcd_msg.txt","r")
